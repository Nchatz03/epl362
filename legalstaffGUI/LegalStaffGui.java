...
//	}
	...
	private void initialize() {
		...
		lblCase.setBounds(12, 93, 56, 16);
		panel_1.add(lblCase);
		
		text_case = new JTextField();
		text_case.setBounds(12, 122, 233, 99);
		panel_1.add(text_case);
		text_case.setColumns(10);
		
		JComboBox comboBox_le = new JComboBox();
		comboBox_le.setModel(new DefaultComboBoxModel(new String[] {"Legal Opinion 1", "Legal Opinion 2", "Legal Opinion 3", "Recommendation  1", "Recommendation  2", "Recommendation  3"}));
		...
		btnUpdate.addActionListener(new ActionListener() {
			...
			String s=	comboBox_le.getSelectedItem().toString();
				...
				if(le.isRecommedationTheSame(s, ca_id)) {
					JOptionPane.showMessageDialog(null, "Same Recommendation", "Message", JOptionPane.INFORMATION_MESSAGE);

				}
				else if (le.isUnwilling(le.getClients(ca_id), s)) {
					
					JOptionPane.showMessageDialog(null, "Client is Unwilling for that Strategy", "Message", JOptionPane.INFORMATION_MESSAGE);
		...
		btnInsert.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String cas=text_case.getText();
				int law_id =Integer.parseInt(lawyer_id.getText());
				boolean bo =le.isCaseTheSame(cas, cli_id);
				
				if (!le.checkClientExist(cli_id)) {
					JOptionPane.showMessageDialog(null, "Client not exist", "Problem", JOptionPane.INFORMATION_MESSAGE);
				}
					...
					JOptionPane.showMessageDialog(null, "Case insertion confirm.", "Succeed", JOptionPane.INFORMATION_MESSAGE);
				}
				else {
		...
		btnInsert.setBounds(56, 246, 97, 25);
		...
		btnInsert_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				le.insertRequest(i, req_name.isSelected(), req_sur.isSelected(), req_addre.isSelected(), req_phone.isSelected(), req_dbo.isSelected());
		...
		un_como_reco.setModel(new DefaultComboBoxModel(new String[] {"Recommendation  1", "Recommendation  2", "Recommendation  3", "Legal Opinion 1", "Legal Opinion 2", "Legal Opinion 3"}));
	...
	public void cliDispute_transaction(LegalStaffUI le) {
		Object rowData[][] = null; // {{1,1,1,1,1,1,1}};
		String columns[] = null;
		columns = new String[] { "DisputeTrans_ID", "Client ID", "Date", "Case", "Recommendation or Legal Opinion","Lawyer ID" };
		//
		ArrayList<Object[]> clientsList = new ArrayList<Object[]>();
