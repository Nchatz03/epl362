...
public class LegalStaffUIImpl implements LegalStaffUI {
		...
		try {
			...
			while(result.next()){
				...
				Q.add(new Object[]{result.getString("ID"),result.getString("Name"),result.getString("Surname"),result.getString("Sex"), result.getString("DOB")
						 , result.getString("NeedsUpdate"),result.getString("DropIn"),result.getString("Telephone"),result.getString("Address")});
	...
	public boolean changeCase(String CaseTrans,int ClientID) {
		...
		String query = "UPDATE clients SET CaseTrans = ? , NeedsUpdate = 0 WHERE ID = ? AND Status = 'ACTIVE'";
	...
	public boolean viewLegalStrategies(ArrayList<Object[]> Q){
		...
		try {
			...
			while (result.next()) {
				//System.out.print(result.getString("LegalStrategy")+result.getString("SideEffects"));
	...
	public boolean getNeedsUpdate(ArrayList<Object[]> Q){
		try {
			...
			while (result.next()) {
				//System.out.print(result.getString("ID")+ result.getString("Name")+result.getString("Surname")
			...
			while (result.next()) {
               ...
               //        +result.getString("CaseTrans")+ result.getString("Rec_LegalOp"));
				...
				Q.add(new Object[] {result.getString("ID"), result.getString("Name"),result.getString("Surname"),
						result.getString("Legal"),result.getString("CaseTrans"), result.getString("Rec_LegalOp")});
	...
	public boolean addDisputeTransaction(int DisputeID, int ClientID, String Date, String Description, String LegalStrategy){
		String query = "INSERT INTO disputetransaction(Dispute_ID,Client_ID,DisputeDate,Description,LegalStrategy) Values (?,?,?,?,?)";
		
		 
	      
		 try {
			PreparedStatement preparedStmt = connection.prepareStatement(query);
			preparedStmt.setInt (1, DisputeID);
			preparedStmt.setInt (2, ClientID);
			preparedStmt.setString (3, Date);
		    preparedStmt.setString (4, Description);
		    preparedStmt.setString (5, LegalStrategy);
		...
		try {
			...
			while (result.next()) {
				Q.add(new Object[] {result.getString("ID"),result.getString("Client_ID"),
						result.getString("DisputeDate"),result.getString("Description")});
	...
	public boolean getDisputeTransaction(ArrayList<Object[]> Q){
		try {
			...
			while (result.next()) {
				Q.add(new Object[] {result.getString("Dispute_ID"),result.getString("Client_ID"),result.getString("DisputeDate")
						,result.getString("Description"),result.getString("LegalStrategy")});
